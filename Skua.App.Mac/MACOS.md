...
Download the latest Apple Silicon build from [GitHub Releases](https://github.com/Crimson341/qexec/releases/latest), or use in-app **Update** on a packaged `qexec.app`: it downloads that zip, replaces this app, and relaunches (it does not open GitHub). The running app checks GitHub at launch, when the window is focused, and every ten minutes, so a build that was current at startup can still see a newer zip. Replacing a Desktop copy uses Finder when macOS TCC blocks `ditto` (`Operation not permitted`). Pushes to `main` run `.github/workflows/macos-release.yml`, which sets `QEXEC_VERSION` to the release number (`0.2.<run>`), calls `./build-macos.sh` on a GitHub-hosted Mac, and publishes a non-prerelease zip so `/releases/latest` resolves. That bake writes `version.json`, rewrites the bundled `package.json` (Electron `app.getVersion()`), and puts the GitHub tag plus commit SHA in the title bar. Extract it, move `qexec.app` to Applications, and follow the included setup instructions. The application is ad-hoc signed and not notarized. OpenJDK is required for automatic map pickup inspection; Homebrew OpenJDK is detected automatically. A manual rebuild is available from the Actions tab (**Mac app release** → Run workflow).
