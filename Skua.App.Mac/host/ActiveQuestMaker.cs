...
public sealed class ActiveQuestMaker(IScriptInterface bot, GearFinder finder, string scriptsRoot)
{
    ...
    public object Snapshot() {
        ...
        return new {type="active-quests",quests=active.Select(q=>Describe(q.ID,q.Name,q.Requirements,
            ...
            q.SimpleRewards.Where(r=>r.Type==2).Select(r=>new {id=r.ID,name=q.Rewards.FirstOrDefault(i=>i.ID==r.ID)?.Name??"Item #"+r.ID}))).ToArray()};
    }
    ...
    public static string ReturnCode(QuestReturnPoint? point) {
        ...
        return "if(bot.ShouldExit) return; bot.Log(\"Objectives complete. Returning to the quest's recorded location…\"); core.Join("+Q(point.Map)+","+Q(cell)+","+Q(pad)+"); if(bot.ShouldExit) return; if(!bot.Player.LoggedIn || bot.Map.Name!="+Q(point.Map)+" || bot.Player.Cell!="+Q(cell)+") throw new InvalidOperationException(\"Could not return to the quest location. Quest was not turned in.\");\n";
    }
    public static string Generate(int questId,int rewardId,IEnumerable<ItemBase> requirements,IEnumerable<GearDrop> drops,Func<string,int,GearShop?> shopFor,ISet<int>? bankOwned=null,IEnumerable<QuestPickup>? pickups=null,bool bankAvailable=true,QuestReturnPoint? returnTo=null)
    {
        ...
        foreach(var r in requirements) {
            ...
            string unbank=r.Temp ? "" : "if(bot.Bank.Loaded && bot.Bank.Contains("+r.ID+")) { core.Unbank("+r.ID+"); for(int sync=0;sync<20 && !bot.Inventory.Contains("+r.ID+","+r.Quantity+") && !bot.ShouldExit;sync++) System.Threading.Thread.Sleep(100); }\n";
            string action;
            if(!r.Temp && bankOwned?.Contains(r.ID)==true) action="";
                ...
                action="core.Join("+Q(pickup.Map)+"); Skua.Core.Scripts.QuestMapPickup."+(pickup.MapItemID>0?"AcquireKnown":"Acquire")+"(bot,"+questId+","+r.ID+","+Q(r.Name)+","+r.Quantity+","+r.Temp.ToString().ToLowerInvariant()+(pickup.MapItemID>0?","+pickup.MapItemID:"")+");";
            else if(drop!=null) action="core.Join("+Q(drop.Map)+"); Skua.Core.Scripts.QuestHunt.Monster(bot,"+questId+","+Q(drop.Monster)+","+r.ID+","+Q(r.Name)+","+r.Quantity+","+r.Temp.ToString().ToLowerInvariant()+");";
