...
public class CoreStory
{
    ...
    public void PreLoad(object _this, [CallerMemberName] string caller = "")
    {
        ...
        string className = _this?.GetType().Name ?? "";
        int classStartIndex = Array.FindIndex(ScriptSlice, line => line.Trim() == $"public class {className}");
        if (classStartIndex < 0)
        {
            Core.Logger($"Quest preload skipped: source for {className} was not found. Quests will load when needed.");
            return;
        }
        int classEndIndex = Array.FindIndex(ScriptSlice, classStartIndex + 1,
            line => line == ScriptSlice[classStartIndex].Substring(0, ScriptSlice[classStartIndex].Length - ScriptSlice[classStartIndex].TrimStart().Length) + "}");
        if (classEndIndex < 0)
        {
            Core.Logger("Quest preload skipped: class boundary was not found. Quests will load when needed.");
            return;
        }
        classEndIndex++;
        ...
        if (methodStartIndex + 1 >= ScriptSlice.Length || ScriptSlice[methodStartIndex + 1].Trim() != "{")
        {
            Core.Logger("Quest preload skipped: unsupported method layout. Quests will load when needed.");
            return;
        }
        string indent = ScriptSlice[methodStartIndex + 1].Split('{')[0];
        int methodEndIndex = Array.FindIndex(ScriptSlice, methodStartIndex + 2, l => l == indent + "}");
        if (methodEndIndex < 0)
        {
            Core.Logger("Quest preload skipped: method boundary was not found. Quests will load when needed.");
            return;
        }
        ...
        foreach (string Line in ScriptSlice)
        {
            ...
            if (!int.TryParse(new string(digits), out int QuestID) || QuestID <= 0)
                continue;
