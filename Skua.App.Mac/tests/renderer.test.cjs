...
test('script controls and elapsed time follow host lifecycle, and logs remain bounded', () => {
  ...
  receive({type:'active-quest-finished',id:42,reward:-1,farmable:true,name:'Quest',message:'Quest is a farming quest. Do the same route again?'});
  assert.equal(elements.get('ledger-repeat').hidden,false,'Farming quests ask before repeating');
  assert.match(elements.get('ledger-repeat-note').textContent,/farming quest/);
  elements.get('ledger-repeat-yes').onclick();
  assert.deepEqual(JSON.parse(ledgerCommands.at(-1)[1]),{id:42,reward:-1,repeat:true},'Yes re-accepts and Auto-does the same quest');
  receive({type:'active-quest-error',message:'Quest locked'});
  assert.equal(elements.get('ledger-repeat').hidden,true,'A later error dismisses the repeat prompt');
  assert.equal(elements.get('ledger-status').textContent,'Quest locked','Ledger failures stay inline without a dialog');
  receive({type:'active-quests',quests:[{id:50,name:'Plant hunt',ready:false,rewards:[],objectives:[{name:'Plant Found',have:1,need:3}],dailyDone:false,member:false,locked:false,blocked:false,farmable:true}]});
  const plantRow=elements.get('ledger-list').children[0].children[0];
  assert.match(plantRow.children[1].textContent,/In progress/);
  assert.match(plantRow.children[1].textContent,/Farm \/ repeatable/);
